...
export default function HeroKPIGrid({ data, ticker }: HeroKPIGridProps) {
  const { currentPrice, prevClose, rsi, macd, ma200, backtestRMSE, dailyHigh, dailyLow, volume } =
    data.summary;
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {TICKER_INFO[ticker] ?? `${ticker} · NASDAQ`}
          </p>
        </div>
        <div className="text-right hidden sm:block">
          <div className="text-xs text-muted-foreground font-mono-data">Last updated</div>
          <div className="text-xs text-foreground font-mono-data">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4 gap-4">
        ...
        <div
          className={`lg:row-span-2 glass-card-cyan rounded-xl p-6 glow-cyan flex flex-col justify-between transition-all duration-300 hover:glow-cyan-strong`}
        >
          <div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <Target
              size={14}
              className={Math.abs(ma200Dev) > 15 ? 'text-warning-amber' : 'text-muted-foreground'}
            />
          </div>
          <div
          ...
          <div className="mt-3">
            <div className="text-xs text-muted-foreground">
              MA200:{' '}
              <span className="font-mono-data text-foreground">{formatPrice(ma200, ticker)}</span>
            </div>
              ...
              className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${
                backtestRMSE < 0.02
                  ...
                  : backtestRMSE < 0.05
                    ...
                    : 'bg-warning/10 text-warning-amber border border-warning/30'
              }`}
            >
              {backtestRMSE < 0.02
                ? 'HIGH CONFIDENCE'
