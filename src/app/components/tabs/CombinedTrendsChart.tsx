...
export default function CombinedTrendsChart({ data, ticker }: Props) {
  ...
  const crossovers = chartData.reduce<Array<{ date: string; type: 'golden' | 'death' }>>(
    (acc, row, i) => {
      if (i === 0) return acc;
          ...
          {[
            {
              ...
              value: formatPrice(data.summary.ma100 - data.summary.ma200, ticker),
              positive: data.summary.ma100 > data.summary.ma200,
            },
            {
              label: 'Spread %',
              value: `${(((data.summary.ma100 - data.summary.ma200) / data.summary.ma200) * 100).toFixed(2)}%`,
              positive: data.summary.ma100 > data.summary.ma200,
            },
            ...
            <div key={`spread-${item.label}`}>
              ...
              <div
                className={`font-mono-data text-sm font-semibold ${item.label.includes('Spread') ? (item.positive ? 'text-positive' : 'text-negative') : 'text-foreground'}`}
              >
                {item.value}
              </div>
            </div>
