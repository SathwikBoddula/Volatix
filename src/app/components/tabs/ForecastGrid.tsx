...
export default function ForecastGrid({ data, ticker }: Props) {
  ...
  const maxGain = Math.max(
    ...forecast.map((f) => ((f.predicted - currentPrice) / currentPrice) * 100)
  );
  ...
  return (
    <div>
      ...
      {/* Summary Banner */}
      <div
        className={`rounded-xl p-4 mb-5 border flex items-center justify-between ${
          isPositiveOutlook
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              From {formatPrice(currentPrice, ticker)} → {formatPrice(finalDay.predicted, ticker)} ·
              Peak at {formatPrice(currentPrice * (1 + maxGain / 100), ticker)}
            </p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {forecast.map((row, idx) => {
              ...
              const signal =
                row.confidence > 75 ? (isUp ? 'Strong Buy' : 'Strong Sell') : isUp ? 'Buy' : 'Sell';
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className="font-mono-data text-sm font-bold text-foreground">
                      {formatPrice(row.predicted, ticker)}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`font-mono-data text-xs font-semibold ${isUp ? 'text-positive' : 'text-negative'}`}
                    >
                      {isUp ? '+' : ''}
                      {deltaFromToday.toFixed(2)}%
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`font-mono-data text-xs ${isPrevUp ? 'text-positive' : 'text-negative'}`}
                    >
                      {isPrevUp ? '+' : ''}
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-semibold px-2 py-0.5 rounded-full border font-mono-data ${signalColor}`}
                    >
                      {signal}
                    </span>
                  </td>
