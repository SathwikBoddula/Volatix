...
import {
  ResponsiveContainer, ComposedChart, Area, Line, XAxis, YAxis, CartesianGrid, Tooltip, ReferenceLine, ReferenceArea
} from 'recharts';
import type { TickerData } from '../../data/mockData';

interface Props { data: TickerData; ticker: string; }

const CustomTooltip = ({ active, payload, label, ticker }: { active?: boolean; payload?: Array<{ name: string; value: number; color: string; dataKey: string }>; label?: string; ticker?: string }) => {
  ...
  const stateColor = rsiVal > 70 ? 'var(--danger)' : rsiVal < 30 ? 'var(--success)' : 'var(--muted-foreground)';
  return (
    <div className="glass-card rounded-lg p-3 border border-border shadow-2xl min-w-[180px]">
      ...
      <div className="mt-2 pt-2 border-t border-border">
        <span className="text-xs font-mono-data font-bold" style={{ color: stateColor }}>{state}</span>
      </div>
    </div>
  );
};
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">14-period Relative Strength Index · overbought/oversold zones</p>
        </div>
      ...
      <div className="glass-card rounded-xl p-4 border border-border mb-4">
        <p className="text-xs text-muted-foreground mb-3 font-semibold uppercase tracking-wide">Price Context</p>
        <ResponsiveContainer width="100%" height={140}>
          <ComposedChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
            ...
            <YAxis tick={{ fill: 'var(--muted-foreground)', fontSize: 10, fontFamily: 'var(--font-mono)' }} tickLine={false} axisLine={false} tickFormatter={v => formatPrice(v, ticker)} width={60} />
            <Area type="monotone" dataKey="close" name="Close" fill="url(#priceGradRSI)" stroke="var(--primary)" strokeWidth={1.5} dot={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="glass-card rounded-xl p-4 border border-border">
        <p className="text-xs text-muted-foreground mb-3 font-semibold uppercase tracking-wide">RSI (14)</p>
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 10 }}>
            ...
            <ReferenceLine y={70} stroke="var(--danger)" strokeDasharray="4 2" strokeWidth={1} label={{ value: 'OB 70', position: 'insideTopRight', fill: 'var(--danger)', fontSize: 10, fontFamily: 'var(--font-mono)' }} />
            <ReferenceLine y={30} stroke="var(--success)" strokeDasharray="4 2" strokeWidth={1} label={{ value: 'OS 30', position: 'insideBottomRight', fill: 'var(--success)', fontSize: 10, fontFamily: 'var(--font-mono)' }} />
            <ReferenceLine y={50} stroke="var(--border)" strokeDasharray="2 4" strokeWidth={1} />
            <Line type="monotone" dataKey="rsi" name="RSI(14)" stroke="var(--primary)" strokeWidth={2} dot={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mt-4">
        {[
          { label: 'Overbought Sessions', count: overboughtCount, pct: ((overboughtCount / chartData.length) * 100).toFixed(1), colorClass: 'text-negative', bgClass: 'bg-danger/10 border-danger/20' },
          { label: 'Neutral Sessions', count: neutralCount, pct: ((neutralCount / chartData.length) * 100).toFixed(1), colorClass: 'text-muted-foreground', bgClass: 'bg-muted/50 border-border' },
          { label: 'Oversold Sessions', count: oversoldCount, pct: ((oversoldCount / chartData.length) * 100).toFixed(1), colorClass: 'text-positive', bgClass: 'bg-success/10 border-success/20' },
        ].map(item => (
