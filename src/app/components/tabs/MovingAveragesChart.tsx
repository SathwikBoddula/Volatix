...
export default function MovingAveragesChart({ data, ticker }: Props) {
  const [visibleMA, setVisibleMA] = useState({ ma100: true, ma200: true, ma250: true, close: true });
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">100-day, 200-day, and 250-day rolling averages</p>
        </div>
      ...
      <div className="glass-card rounded-xl p-4 border border-border">
        <ResponsiveContainer width="100%" height={420}>
          <LineChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 10 }}>
            ...
            {visibleMA.close && (
              <Line type="monotone" dataKey="close" name="Close Price" stroke="var(--foreground)" strokeWidth={1.5} dot={false} opacity={0.6} />
            )}
            {visibleMA.ma100 && (
              <Line type="monotone" dataKey="ma100" name="MA 100" stroke="var(--primary)" strokeWidth={2} dot={false} />
            )}
            {visibleMA.ma200 && (
              <Line type="monotone" dataKey="ma200" name="MA 200" stroke="var(--accent)" strokeWidth={2} dot={false} />
            )}
            {visibleMA.ma250 && (
              <Line type="monotone" dataKey="ma250" name="MA 250" stroke="var(--warning)" strokeWidth={1.5} strokeDasharray="5 3" dot={false} />
            )}
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
        {[
          { label: 'MA 100', value: data.summary.ma100, period: '100-day', color: 'var(--primary)', borderClass: 'border-primary/20' },
          { label: 'MA 200', value: data.summary.ma200, period: '200-day', color: 'var(--accent)', borderClass: 'border-accent/20' },
          { label: 'MA 250', value: data.summary.ma250, period: '250-day', color: 'var(--warning)', borderClass: 'border-warning/20' },
        ].map(item => {
          const dev = ((data.summary.currentPrice - item.value) / item.value) * 100;
          return (
            <div key={`macard-${item.label}`} className={`glass-card rounded-xl p-4 border ${item.borderClass}`}>
              ...
              <div className="font-mono-data text-2xl font-bold text-foreground">{formatPrice(item.value, ticker)}</div>
              <div className={`text-xs font-mono-data mt-1 ${dev >= 0 ? 'text-positive' : 'text-negative'}`}>
                Price is {dev >= 0 ? '+' : ''}{dev.toFixed(2)}% {dev >= 0 ? 'above' : 'below'} {item.period} avg
              </div>
            </div>
          );
        })}
